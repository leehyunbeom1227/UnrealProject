	...
	for (int32 i = 0; i <= m_DivideCount; ++i)
	{
		...
		if (m_MeshVisible)
		{
			if (m_DivideCount != m_PrevDivide || MeshEmpty)
			{
				...
				FTransform MeshTransfrom = MeshCom->GetRelativeTransform();

				MeshTransfrom.SetLocation(Point);
				MeshTransfrom.SetScale3D(FVector(0.1f, 0.1f, 0.1f));

				MeshCom->SetRelativeTransform(MeshTransfrom);
